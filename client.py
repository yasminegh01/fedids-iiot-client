import os, argparse, threading, time, random, configparser, requests
import numpy as np
...
REAL_WORLD_IPS = [
    ...
    "208.67.222.222", "213.73.91.35", "41.231.53.2", "200.1.122.10"
]
ATTACK_TYPES = ['Backdoor','DDoS_ICMP','DDoS_TCP','MITM','Port_Scanning','Ransomware']

# --- Fonctions Utilitaires ---
def get_device_api_key(config_file: str):
...
def background_tasks(api_key: str, stop_event: threading.Event):
    ...
    while not stop_event.is_set():
        # Heartbeat
        try:
            requests.post(f"{API_URL}/api/devices/heartbeat",
                          json={"api_key": api_key}, timeout=5)
        ...
        if random.random() > 0.7:
            ...
            try:
                requests.post(f"{API_URL}/api/attacks/report",
                              json=attack, timeout=5)
                print(f"🛑 [Background] Attack '{attack['attack_type']}' "
                      f"from {attack['source_ip']} reported.")
            except:
                pass
...
def create_model():
    """Exemple de modèle CNN+LSTM (ajuste selon ton besoin)."""
    model = tf.keras.Sequential([
        tf.keras.layers.Conv1D(64, 3, activation="relu", input_shape=(TIME_STEPS, NUM_FEATURES)),
